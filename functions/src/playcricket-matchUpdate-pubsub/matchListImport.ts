...
export class MatchListImport {
    ...
    constructor(private afs = admin.firestore(), private plycrctAPICall = new PlayCricketAPICall()) {
        this.updateMatchList({test:'test'})
    }
    ...
    public importMatchList(seasonID:string, siteID:string, apiToken:string){
            ...
            const playcrictetData = this.plycrctAPICall.playCricketApiCall(seasonID, siteID, apiToken); //Get data from play cricket
              //  .then( APIResponse => {
            ...
            const dbFixtureData = this.afs.collection('Fixtures').where('season', '==', 2020).get();
            Promise.all([playcrictetData, dbFixtureData])
                .then(
    ...
    private updateMatchList(matchListData: any): WriteBatch {
        const matchListBatch = this.afs.batch();
        matchListData.matches.forEach(element => {
            const dRef:DocumentReference = this.afs.doc('Fixtures/' + element.id)
